  ...
  try {
    ...
    if (session.role !== UserRole.MANAGER && session.role !== UserRole.ADMIN) {
      return NextResponse.json(
        { success: false, error: 'Only managers and admins can approve goal sheets' },
        { status: 403 },
      );
    }
    ...
    // Check if manager is approving their direct reports (MANAGER role)
    if (session.role === UserRole.MANAGER) {
      ...
      if (!isDirectReport) {
        return NextResponse.json(
          { success: false, error: 'Can only approve your direct reports' },
          { status: 403 },
        );
      }
    }
    ...
    if (goalSheet.status !== GoalSheetStatus.SUBMITTED) {
      return NextResponse.json(
        { success: false, error: 'Only SUBMITTED goal sheets can be approved' },
        { status: 400 },
      );
    }
