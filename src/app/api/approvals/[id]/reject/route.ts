  ...
  try {
    ...
    if (session.role !== UserRole.MANAGER && session.role !== UserRole.ADMIN) {
      return NextResponse.json(
        { success: false, error: 'Only managers and admins can reject goal sheets' },
        { status: 403 },
      );
    }
    ...
    if (session.role === UserRole.MANAGER) {
      ...
      if (!isDirectReport) {
        return NextResponse.json(
          { success: false, error: 'Can only reject goal sheets for your direct reports' },
          { status: 403 },
        );
      }
    }
    ...
    if (goalSheet.status !== GoalSheetStatus.SUBMITTED) {
      return NextResponse.json(
        { success: false, error: 'Only SUBMITTED goal sheets can be rejected' },
        { status: 400 },
      );
    }
